    ...
    async LoginWithToken() {
        await this.http.post(URLConstant.LoginWithToken, { ModuleCode: environment.Module }, this.SpinnerOptions).toPromise().then(
            async (response) => {
                ...
                await this.http.post(URLConstant.GetAllActiveRefFormByRoleCodeAndModuleCode, { RoleCode: response["Identity"].RoleCode, ModuleCode: environment.Module }, this.SpinnerOptions).toPromise().then(
                    (response) => {
