...
export class UcTempComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getJSON(this.tempPagingObj.urlJson).subscribe(data => {
      ...
      if (this.configuration.headerListTemp == undefined || this.configuration.headerListTemp.length == 0) {
        this.configuration.headerListTemp = this.configuration.headerList;
      }
      if (this.configuration.bodyListTemp == undefined || this.configuration.bodyListTemp.length == 0) {
        this.configuration.bodyListTemp = this.configuration.bodyList;
      }
    });
    ...
    this.inputSearchObj.whereValue = this.tempPagingObj.whereValue;
  ...
  Checked(SelectedId: number, isChecked: boolean): void {
    ...
    if (this.listSelectedId.length != this.resultData.Data.length) {
      this.checkboxAll = false;
    } else {
  ...
  TempCheckbox() {
    let TempObj = {
      TempListId: this.tempListId,
      TempListObj: this.tempData
    }
    this.callback.emit(TempObj);
  }
  ...
  addToTemp() {
    if (this.listSelectedId.length != 0) {
      ...
      for (var i = 0; i < this.listSelectedId.length; i++) {
        var object = this.resultData.Data.find(x => x[this.configuration.keyProperty] == this.listSelectedId[i]);
        for (let j = 0; j < this.configuration.bodyListTemp.length; j++) {
          if (this.configuration.bodyListTemp[j].type == "checkbox") {
            object[this.configuration.bodyListTemp[j].property] = false;
          }
        }
